...
export default function Newsletter() {
  ...
  const y = useTransform(progress, [0, .42, .72, 1], [110, 0, 0, -50]);
  const x = useTransform(progress, [0, .42, .72, 1], [24, 0, 0, -12]);
  const scale = useTransform(progress, [0, .42, .72, 1], [.72, 1, 1, 1.04]);
  const copyY = useTransform(progress, [.08, .42, .72, 1], [48, 0, 0, -18]);
  const lightY = useTransform(progress, [0, 1], [160, -160]);
  const line = useTransform(progress, [.08, .55], [0, 1]);

  return (
    <section ref={ref} id="newsletter" className="mx-auto max-w-6xl px-6 py-20 sm:py-24">
      <motion.div data-scroll-layer="newsletter-panel" style={reduced ? undefined : { x, y, scale }} className="relative isolate overflow-hidden rounded-3xl border border-amber/20 bg-[#14100e] p-8 shadow-[0_30px_90px_-45px_#000] sm:p-14">
