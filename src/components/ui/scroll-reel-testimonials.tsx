...
/* ----------------------------------------------------------------
 * ScrollReelTestimonials (dark / molten-amber adaptation)
...
const FEATURED_SHADOW =
  "0 20px 45px -12px rgba(0,0,0,0.65), inset 0 1px 0 rgba(255,255,255,0.08)";
...
function Cell() {
  return (
    <div
      aria-hidden="true"
      className="shrink-0 rounded-xl border border-edge bg-gradient-to-b from-surface-2 to-night blur-[1px]"
...
function Featured({ src, alt }: { src: string; alt?: string }) {
  return (
    <div
      className="relative shrink-0 overflow-hidden rounded-xl bg-surface ring-1 ring-edge"
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(220.99deg, rgba(245,158,11,0) 32%, rgb(245,158,11) 41%, rgb(255,183,77) 47%, rgba(255,107,26,0.5) 54%, rgba(255,107,26,0) 65%)",
        }}
      />
    </div>
  );
...
export function ScrollReelTestimonials({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className={cn(
        "relative flex w-full max-w-[1060px] flex-col items-stretch gap-2.5 overflow-hidden rounded-3xl border border-edge bg-surface/40 outline-none focus-visible:ring-2 focus-visible:ring-amber md:min-h-[320px] md:flex-row",
        className,
      )}
    >
      <div
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between self-stretch px-5 py-7 md:py-10">
        <div className="flex flex-col gap-[9px]">
          <svg
            className="block h-12 w-12 text-amber/40"
            viewBox="0 0 24 24"
            fill="currentColor"
            aria-hidden="true"
          >
        ...
        <div className="mt-6 flex items-center gap-1.5 md:mt-0">
          <button
            ...
            className="grid h-7 w-7 cursor-pointer place-items-center rounded-full border border-edge-strong bg-transparent p-0 text-ink transition-[opacity,transform] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] hover:enabled:scale-[1.08] active:enabled:scale-[0.94] disabled:cursor-default disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber"
          >
            <svg
